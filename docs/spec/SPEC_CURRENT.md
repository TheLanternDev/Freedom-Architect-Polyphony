...
- **Cost log:** append-only `data/cost_log.jsonl` (override: `COST_LOG_PATH`; pola: agent, model, tokeny, koszt USD, brief_hash)
...
5. `main.py` — duży monolit; część tras przeniesiona do `api/routers/meta.py`; SSE/debate nadal w `main.py`
6. ~~Brak auth na żadnym endpoincie~~ — JWT (`ARCHITEKT_JWT_SECRET`), nagłówek serwisowy BFF, lub legacy Bearer `ARCHITEKT_API_KEY` — patrz `api/http_guard.py`, `docs/SECURITY_PRODUCTION.md`
7. Klucz API w przeglądarce (`VITE_ARCHITEKT_API_KEY` / localStorage) — **niezalecane** w prod; preferuj BFF + JWT lub nagłówek serwisowy (`docs/COMPLIANCE_PRIVACY.md`)
