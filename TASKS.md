...
- [ ] **N1: Szyfrowanie danych at-rest** - lokalny SQLite plaintext trzyma dane o zdrowiu. SQLCipher / file-encryption z keychain OS, lub jawna akceptacja klienta. (🔴 [C]) — *najmniejszy ruch: wybór mechanizmu*. Most na pierwszą falę testerów: disclaimer w onboardingu zamiast SQLCipher — patrz `docs/PACZKA_TESTOWA_STATUS_2026-07-07.md`.
...
- [ ] **N4: Podpisywanie + notaryzacja paczki** - `signingIdentity`/`certificateThumbprint` (macOS Developer ID + Win cert). (🔴 [P]+[C]) — CI (`tauri-release.yml`) już akceptuje sekrety i ma `skip_sign_check` jako świadomy most bez certów dla ≤10 zaufanych testerów; decyzja o zakupie kont/certów zostaje otwarta.
...
- [x] **B1: Paczka testowa self-contained — sidecar (2026-07-07)** - Zamyka blocker B1 z `docs/REVIEW_I_PACZKA_TESTOWA_2026-06-25.md` na poziomie kodu: backend zamrożony PyInstallerem (`boxed_entry.py`, `scripts/build-backend-sidecar.sh`/`.ps1`) i wpięty jako Tauri sidecar (`bundle.externalBin`, `lib.rs` `spawn_backend_sidecar`) — `.dmg`/`.msi` nie wymaga już Pythona/Node/repo u testera, ikona na pulpicie wystarcza. `env_bootstrap.py` generuje i trwale zapisuje `ARCHITEKT_JWT_SECRET` + ścieżki danych (`app_data_dir()` per-OS) przy pierwszym starcie — zero ręcznej edycji `.env`. Naprawiony przy okazji: dynamiczny loader `env_bootstrap` w `main.py`/`config/__init__.py` cicho failował pod frozen (żywy bug, nigdy jeszcze nie odpalony). Naprawiony finding: `config/patryk_identity.json` (prywatny plik, AKSJOMAT 3) nie był wykluczony z `pack-founders-archive.sh` — teraz wykluczony + twarda walidacja. CI (`tauri-release.yml`) dostał brakujący job `build-windows` (był tylko macOS) + krok budowy sidecara na obu OS-ach. Pełny status i co zostaje po stronie decyzji biznesowych (podpis, SQLCipher): `docs/PACZKA_TESTOWA_STATUS_2026-07-07.md`. **Nie zweryfikowane empirycznie** — sandbox bez dostępu do PyPI i bez macOS/Windows; pierwszy realny build robi Patryk lokalnie lub przez CI.
