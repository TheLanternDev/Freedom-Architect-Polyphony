...
def test_anthropic_omits_temperature_for_opus_47():
    assert lp.anthropic_omits_temperature("claude-opus-4-7") is True
    assert lp.anthropic_omits_temperature("claude-opus-4-7-20250514") is True
...
def test_anthropic_keeps_temperature_for_typical_sonnet(monkeypatch):
    ...
    assert lp.anthropic_omits_temperature("claude-haiku-4-6") is False
